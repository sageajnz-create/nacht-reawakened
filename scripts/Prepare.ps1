...
$powerups = Replace-Once $powerups 'powerup = spawn ("script_model", drop_point + (0,0,40));' "trace = bullettrace( drop_point + (0,0,80), drop_point + (0,0,-1000), false, undefined );`n`tdrop_origin = drop_point + (0,0,22);`n`tif ( isdefined( trace ) && isdefined( trace[`"position`"] ) && trace[`"fraction`"] < 1 )`n`t`tdrop_origin = trace[`"position`"] + (0,0,22);`n`tpowerup = spawn (`"script_model`", drop_origin);"
